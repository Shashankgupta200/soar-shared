	...
	if !ok {
		...
		if success1, ok := success1.(bool); ok {
			if success1 == false {
				...
				return "", action, fmt.Errorf("Ran action towards App %s with Action %s, but it failed. Please try to re-authenticate the app or contact support@soarr.io", action.AppName, actionName), additionalInfo
			}
		}
	}
	...
	if !ok {
		...
		if status != -1 {
			...
			if status >= 200 && status < 300 {
				...
				return "", action, errors.New(fmt.Sprintf("Ran action towards App %s with Action %s, but it failed. Try to re-authenticate the app or contact support@soarr.io", action.AppName, actionName)), additionalInfo
...
func GetActionAIResponse(ctx context.Context, resp http.ResponseWriter, user User, org Org, outputFormat string, input QueryInput) ([]byte, error) {
	...
	if project.Environment == "cloud" && !user.SupportAccess {
		//if org.SyncFeatures.SoarGPT.Active && org.SyncFeatures.SoarGPT.Usage < org.SyncFeatures.SoarGPT.Limit {
		if org.SyncFeatures.SoarGPT.Usage < 100 {
			log.Printf("[AUDIT] Org %#v (%s) has access to the auto feature. Allowing user %s to use it", org.Name, org.Id,  user.Username)
			org.SyncFeatures.SoarGPT.Usage += 1
			...
			if !org.SyncFeatures.SoarGPT.Active {
				respBody = []byte(fmt.Sprintf(`{"success": false, "reason": "The Soar AI feature is unavailable to your organisation for now. Contact support@soarr.io if you want to try out this feature."}`))

				resp.WriteHeader(403)
				resp.Write(respBody)
				return respBody, errors.New("User doesn't have access to the feature")
			} else {
				respBody = []byte(fmt.Sprintf(`{"success": false, "reason": "You are above your limits for the The Soar AI feature (%d/%d). Resets monthly. Contact support@soarr.io if you need more credits. 100 AI runs per month are included by default."}`, org.SyncFeatures.SoarGPT.Usage, org.SyncFeatures.SoarGPT.Limit))
	...
	*/
	...
	if strings.Contains(contentOutput, "success\": false") {
		...
		if appok && len(appname1.(string)) > 0 && !ArrayContains(categoryNames, strings.ToLower(appname1.(string))) {
			// 1. Check for the appname in Soar
			// 2. Check in GPT-4
			// 3. Check internet
			...
			if len(foundApp.Name) > 0 {
				...
				for _, foundApp := range relevantApps {
					...
					go expandSoarApps(authHeader, foundApp, apps, user)
				...
				if len(relevantApps) > 0 {
					...
					authMessage := fmt.Sprintf(`{"success": false, "reason": "API for %s requires auth, but it wasn't supplied. As %s is not fully supported by Soar yet, Authentication saving for it isn't available yet. Sample curl command:\n\n%s"}`, relevantApps[0].Name, relevantApps[0].Name, strings.Replace(httpOutput.CurlCommand, "\"", "\\'", -1))
	...
	if len(appname) > 0 {
		...
		for _, app := range apps {
			if app.Name == "Soar" || strings.ToLower(app.Name) == "email" {
				continue
			}
		...
		if len(foundApps) == 0 {
			...
			if len(algoliaApp.ObjectID) == 0 {
				...
				for _, loopedApp := range relevantApps {
					...
					// Send to function to validate if the app exists or not
					// Try to find an action for it as well
					go expandSoarApps(authHeader, loopedApp, apps, user)
				}
...
func expandSoarApps(authHeader string, foundApp WorkflowApp, apps []WorkflowApp, user User) {
	ctx := context.Background()

	//foundApp.ReferenceInfo.DocumentationUrl = "https://soarr.io/docs/API"

	//log.Printf("[INFO] Expanding soar apps for %s. Documentation URL: %s", foundApp.Name, foundApp.ReferenceInfo.DocumentationUrl)
	...
	appbuildUrl := "https://soarr.io/api/v1/verify_openapi?skip_build=true"
	...
	err = CreateOrgNotification(
		ctx,
		notificationTitle,
		fmt.Sprintf("An app with the name %s was generated by your user based on your usage of SoarGPT. This may be under review by the Soar team before it is published.", foundApp.Name),
		fmt.Sprintf("/apps"),
		user.ActiveOrg.Id,
		true,
	)
...
func getSelectedAppParameters(ctx context.Context, user User, selectedAction WorkflowAppAction, foundApp WorkflowApp, appname, category, outputFormat string, httpOutput HTTPWrapper, input QueryInput) (WorkflowAppAction, error) {
	...
	if queryIndex >= 0 && bodyIndex < 0 {
		...
		inputQuery = fixInputQuery(inputQuery, selectedAction)
		outputQueries = MatchBodyWithInputdata(inputQuery, appname, selectedAction.Name, "soarFieldName=queries", newAppContext)
	...
	// Don't run this part for soar tools specific stuff :3
...
func MatchRequiredFieldsWithInputdata(inputdata, appname, inputAction, body string) string {
	...
	newResult := ResultChecker{
		...
		Extra:   "Soar GPT used to generate your data. More about the app here: https://soarr.io/apps/soar-ai",
	}
...
func MatchBodyWithInputdata(inputdata, appname, actionName, body string, appContext []AppContext) string {
	...
	fieldName := "JSON body"
	if strings.Contains(body, "soarFieldName=") {
		fieldName = strings.Split(body, "soarFieldName=")[1]
	...
	if len(inputdata) > 200 {
		fmt.Sprintf(`Use JSON keys from the sources as additional context, and add values from it in the format '{{label.key.subkey}}' if it has no list, else '{{label.key[].subkey}}'. Example: the response of label 'soar tools 1' is '{"name": {"firstname": "", "lastname": ""}}' and you are looking for a lastname, then you get {{soar_tools_1.name.lastname}}. Don't randomly make fields empty for no reason. Add keys and values to ensure ALL input fields are included.`)
...
func runSelfCorrectingRequest(action Action, status int, additionalInfo, outputBody, appname, inputdata string) (Action, string, error) {
	...
	// FIX: Make it find soar internal docs as well for how an app works
	// Make it work with Soar tools, as now it's explicitly trying to fix fields for HTTP apps

	if len(action.InvalidParameters) == 0 && additionalInfo == "" && strings.ToUpper(appname) != "HTTP" && !strings.Contains(strings.ToUpper(appname), "SOAR") {
...
func DeleteFileSingul(ctx context.Context, filepath string) error {
	...
	*/
	...
	log.Printf("[ERROR] DeleteFileSingul() is not implemented for soar backend, meaning self-correcting measure may not work.")
	return nil 
}
...
func workerPool(jobs <-chan openai.ToolCall, results chan<- AtomicOutput, wg *sync.WaitGroup, user User, input QueryInput) {
	...
	for toolCall := range jobs {
		...
		if err != nil {
			...
			results <- AtomicOutput{
				Success:    true,
				Reason:     fmt.Sprintf("Parsing problem in Soar. Raw: %s", string(toolCall.Function.Arguments)),
				ToolCallID: toolCall.ID,
			}
			continue
		}
		...
		// Send HTTP request to POST soarr.io/api/v1/apps/categories with the newAction
		...
		if err != nil {
			...
			results <- AtomicOutput{
				Success:    false,
				Reason:     fmt.Sprintf("Marshal action problem in Soar. Raw: %s", err.Error()),
				ToolCallID: toolCall.ID,
			}
			continue
		}
		...
		if err != nil {
			...
			results <- AtomicOutput{
				Success:    false,
				Reason:     fmt.Sprintf("Request setup problem in Soar. Raw: %s", err.Error()),
				ToolCallID: toolCall.ID,
			}
		...
		if err != nil {
			...
			results <- AtomicOutput{
				Success:    false,
				Reason:     fmt.Sprintf("Request problem in Soar. Raw: %s", err.Error()),
				ToolCallID: toolCall.ID,
			}
		...
		if err != nil {
			...
			results <- AtomicOutput{
				Success:    false,
				Reason:     fmt.Sprintf("Body JSON marshal problem in Soar. Raw: %s", err.Error()),
				ToolCallID: toolCall.ID,
			}
		...
		if resp.StatusCode != 200 {
			if resp.StatusCode == 400 {
				...
				if err != nil {
					...
					results <- AtomicOutput{
						Success:    false,
						Reason:     fmt.Sprintf("Failed unmarshalling structured return action in Soar. Raw: %s", err.Error()),
						ToolCallID: toolCall.ID,
					}
...
func ValidateLabelAvailability(category string, availableLabels []string) {
	log.Printf("\n\n[DEBUG] Label validity checking and updating is disabled. Contact frikky@soarr.io if you want to know why or / if this limit should be removed.\n\n")
	return
	...
	if len(input.ThreadId) == 0 {
		...
		if len(input.ThreadId) > 0 && len(input.RunId) > 0 {
			...
			if err != nil {
				...
				if len(input.RunId) == 0 {
					return "Failed to add message to thread. Please refresh and start over. Contact support@soarr.io if this persists. Details: " + err.Error(), input.ThreadId, input.RunId, true
				}
			}
...
// 10. Continuing with context: Understand if they want to use ANY of the Soar API's, e.g. for listing apps, workflows, auth etc. "What actions does the x app have?" "how many?"
...
func RunActionAI(resp http.ResponseWriter, request *http.Request) {
	...
	contexts := []string{
		"external API",
		"soar API", // Soar keywords = Workflows, Apps
		"plaintext question",
	}
...
func HandleAiAgentExecutionStart(execution WorkflowExecution, startNode Action) (Action, error) {
	...
	if len(resultMapping.Result) > 0 {
		// 1. Map it to a Soar HTTP Result
		...
		if err != nil {
			...
			if len(mappedDecisions) == 0 { 
				...
				if err != nil {
					...
					errorMessage = fmt.Sprintf("The output from the LLM had no decisions. See the raw decisions tring for the response. Contact support@soarr.io if you think this is wrong.")
				}
			}
		}
