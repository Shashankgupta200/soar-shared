...
func GetAppbase() ([]byte, []byte, error) {
	...
	if err != nil {
		...
		githubUrl := "https://raw.githubusercontent.com/Soar/app_sdk/refs/heads/main/soar_sdk/soar_sdk.py"
...
func MakePythoncode(swagger *openapi3.Swagger, name, url, method string, parameters, optionalQueries, headers []string, fileField string, api WorkflowApp, handleFile bool) (string, string) {
	...
	data := fmt.Sprintf(`    def %s(self%s):
        ...
        if len(contentType) > 0:
       	...
       	if not found:	
            request_headers["User-Agent"] = "Soar Automation"
        ...
        try:
            ...
            self.action["parameters"].append({
                "name": "soar_request_url",
                "value": f"{url}",
            })
            self.action["parameters"].append({
                "name": "soar_request_curl",
                "value": f"{parsed_curl_command}",
            })
            self.action["parameters"].append({
                "name": "soar_request_headers",
        ...
        try:
            ...
            for item in self.action["parameters"]:
                if item["name"] == "soar_response_status":
                    found = True
                    break

            if not found:
                self.action["parameters"].append({
                    "name": "soar_response_status",
                    "value": f"{ret.status_code}",
                })
                self.action["parameters"].append({
                    "name": "soar_response_length",
                    "value": f"{len(ret.text)}",
                })
                self.action["parameters"].append({
                    "name": "soar_request_cookies",
...
func ValidateParameterName(name string) string {
	...
	for _, item := range invalid {
		if item == name {
			...
			newname = fmt.Sprintf("%s_soar", item)
			break
		}
	}
...
func HandleConnect(swagger *openapi3.Swagger, api WorkflowApp, extraParameters []WorkflowAppActionParameter, path *openapi3.PathItem, actualPath string, optionalParameters []WorkflowAppActionParameter) (WorkflowAppAction, string) {
	...
	if strings.Contains(baseUrl, "_soar_replace_") {
		//log.Printf("[DEBUG] : %s", baseUrl)
		m := regexp.MustCompile(`_soar_replace_\d+`)
...
func DownloadDockerImageBackend(topClient *http.Client, imageName string) error {
	// Check environment SOAR_AUTO_IMAGE_DOWNLOAD
	if os.Getenv("SOAR_AUTO_IMAGE_DOWNLOAD") == "false" {
		log.Printf("[DEBUG] SOAR_AUTO_IMAGE_DOWNLOAD is false. NOT downloading image %s", imageName)
		return nil
	}
	...
	if strings.Contains(baseUrl, "ngrok") || strings.Contains(baseUrl, "soarr.io") || strings.Contains(baseUrl, ".run.app") {
	...
	if len(baseTag) > 1 {
		...
		// Remapping
		ctx := context.Background()
		dockercli.ImageTag(ctx, imageName, fmt.Sprintf("frikky/soar:%s", tag))
		dockercli.ImageTag(ctx, imageName, fmt.Sprintf("registry.hub.docker.com/frikky/soar:%s", tag))

		downloadedImages = append(downloadedImages, fmt.Sprintf("frikky/soar:%s", tag))
		downloadedImages = append(downloadedImages, fmt.Sprintf("registry.hub.docker.com/frikky/soar:%s", tag))
