...
// Soar is an automation platform for security and IT. This app and the associated scopes enables us to get information about a user, their mailbox and eventually subscribing them to send pub/sub requests to our platform to handle their emails in real-time, before controlling how to handle the data themselves.
...
// FIXME:
// 1. Should find contributions to Soar repo's for the user
...
func HandleNewGithubRegister(resp http.ResponseWriter, request *http.Request) {
	...
	repositories := map[string]string{
		"frikky/soar":           "core",
		"frikky/soar-shared":    "core",
		"soar/soar-docs":     "docs",
		"soar/soar-apps":     "apps",
		"soar/openapi-apps":     "apps",
		"soar/soar-usecases": "workflows",
	}
...
func MakeGmailSubscription(ctx context.Context, client *http.Client, folderIds []string) (SubResponse, error) {
	...
	resource := "projects/soarr/topics/gmail_testing"
...
func HandleCreateGmailSub(resp http.ResponseWriter, request *http.Request) {
	...
	for {
		...
		callbackUrl := "https://soarr.io"
		if len(os.Getenv("SOAR_GCEPROJECT")) > 0 && len(os.Getenv("SOAR_GCEPROJECT_LOCATION")) > 0 {
			callbackUrl = fmt.Sprintf("https://%s.%s.r.appspot.com", os.Getenv("SOAR_GCEPROJECT"), os.Getenv("SOAR_GCEPROJECT_LOCATION"))
		}

		if len(os.Getenv("SOAR_CLOUDRUN_URL")) > 0 {
			callbackUrl = os.Getenv("SOAR_CLOUDRUN_URL")
		}
