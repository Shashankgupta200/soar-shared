		...
		if int64(AlertThreshold.Count) < orgStatistics.MonthlyAppExecutions && AlertThreshold.Email_send == false {
			...
			for _, user := range org.Users {
				if user.Role == "admin" {
					...
					// 	BccAddress = []string{"support@soarr.io", "jay@soarr.io"}
					// }
					...
					// 	Body:    fmt.Sprintf("You have reached the threshold limit of %v percent Or %v app executions run. Please login to soar and check it.", AlertThreshold.Percentage, AlertThreshold.Count),
					// }
					...
					err = sendMailSendgridV2(
						[]string{user.Username, "support@soarr.io", "jay@soarr.io"},
						Subject,
						substitutions,
						false,
